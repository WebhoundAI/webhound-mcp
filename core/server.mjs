...
const TOOL_OUTPUT_SCHEMAS = Object.freeze({
  ...
  webhound_onboarding: toolOutputSchema('webhound_onboarding', {
    ...
    flow_version: z.literal(2).optional(),
    ...
    topic_change_behavior: z.string().optional(),
    ...
    immediate_next_message: z.string().optional(),
    setup_flow: z.array(z.string()).optional(),
    suggested_agent_rules: FLEX_OBJECT.optional(),
    billing_url: z.string().nullable().optional(),
  }),
  ...
  webhound_account: toolOutputSchema('webhound_account', {
    ...
    billing: FLEX_OBJECT.optional(),
    research_harness: z.string().optional(),
    credit_balance_usd: z.number().optional(),
    available_credit_balance_usd: z.number().optional(),
    reserved_credit_balance_usd: z.number().optional(),
    credit_availability_verified: z.boolean().optional(),
    can_start_default_paid_run: z.boolean().optional(),
    can_start_default_run: z.boolean().optional(),
    can_start_any_onboarding_paid_run: z.boolean().optional(),
    can_start_any_onboarding_run: z.boolean().optional(),
    can_start_standard_onboarding_paid_run: z.boolean().optional(),
    can_start_standard_onboarding_run: z.boolean().optional(),
    minimum_supported_budget_usd: z.number().optional(),
...
const TOOL_SUCCESS_CONTRACTS = Object.freeze({
  ...
  webhound_onboarding: data => (
    ...
    && data.flow_version === 2
    && hasText(data.onboarding_version)
    && data.flow_sequence?.canonical === 'agent_playbook.conversation_flow'
    && data.flow_sequence?.setup_flow_role === 'reference_only'
    && data.flow_sequence?.next_action_role === 'entry_instruction_only'
    && hasText(data.message)
    && Array.isArray(data.choices)
    && hasText(data.next_action)
    && isRecord(data.account_state)
    && isRecord(data.budget_model)
    && isRecord(data.budget_summary)
    && Number(data.budget_summary.minutes_per_dollar) === 15
    && Number(data.budget_summary.standard_default_budget_usd) === 5
    && Number(data.budget_summary.standard_default_research_minutes) === 75
    && data.budget_summary.tiers_are_caps === false
    && Array.isArray(data.budget_summary.recommended_starting_points)
    && data.budget_summary.recommended_starting_points.some(tier => Number(tier?.amount_usd) === 20 && Number(tier?.estimated_minutes) === 300)
    && data.budget_model?.dollars_to_minutes?.['$20'] === 'about 300 minutes (5 hours)'
    && isRecord(data.agent_playbook)
    && isRecord(data.user_facing_guidance)
    && hasText(data.immediate_next_message)
    && Array.isArray(data.setup_flow)
  ) ? null : 'the rich onboarding response, budget mapping, or guided flow is incomplete',
  webhound_help: data => (
    hasText(data.topic)
    && hasText(data.answer)
    && data.no_spend === true
...
const HOSTED_ONBOARDING_CLIENTS = new Set(['hosted', 'manus']);
const LOCAL_ONBOARDING_CLIENTS = new Set([
  'codex',
  'claude_code',
  'cursor',
  'opencode',
...
const ONBOARDING_MINUTES_PER_DOLLAR = 15;
const MINIMUM_ONBOARDING_BUDGET_USD = 1;
const STANDARD_ONBOARDING_BUDGET_USD = 5;
const STANDARD_ONBOARDING_MINUTES = STANDARD_ONBOARDING_BUDGET_USD * ONBOARDING_MINUTES_PER_DOLLAR;
const ONBOARDING_BUDGET_STARTING_POINTS = Object.freeze([
...
const ONBOARDING_TOPIC_CHANGE_BEHAVIOR = 'If the user asks a different question, immediately drop onboarding and answer the new request. Do not mention onboarding as pending. Resume only if the user explicitly asks to continue it.';
...
function canonicalOnboardingBudgetModel(value = {}) {
  ...
  return {
    ...
    agent_language: budgetModel.agent_language
      || 'Users can ask for a Webhound run by dollars or minutes. Convert minutes to dollars using about 15 minutes per $1. Present $2, $5, $10, and $20 as starting points, not maximums; larger custom budgets are available for longer, deeper research.',
    meaning: budgetModel.meaning
      || 'The budget is research depth. More budget gives Webhound more time to search, read, write, and verify before final output.',
    completion_signal: budgetModel.completion_signal
      || 'A run is done only when webhound_watch.done is true. output_ready or partial working notes before done=true are not final completion signals.',
  };
}
...
function compactLocalOnboardingWorkspaceRules(value = {}, fallbackSuggestedRules = {}) {
  ...
  const suggestedRules = isRecord(rules.suggested_rules)
    ? rules.suggested_rules
    : canonicalOnboardingAgentRules(fallbackSuggestedRules);
  return {
...
function localOnboardingPlaybook(data, message, canStart, { freeRunRequiresOptIn = false } = {}) {
  ...
  const setupChoiceMessage = canStart
    ? message
    : 'Funding is confirmed. Would you like to set up this workspace first or jump right into the first research run?';
  const fundedFlow = [
  ...
  const fundingPrefix = canStart ? [] : [
    { step: 1, name: freeRunRequiresOptIn ? 'Choose first-run funding' : 'Unblock billing', say: message, choices: freeRunRequiresOptIn ? ['use_included_run', 'billing_ready'] : ['billing_ready'], wait_for_user: true },
    {
      ...
      say: freeRunRequiresOptIn
        ? 'If the user chose use_included_run, treat that as explicit consent for exactly one $5 report or dataset and continue with use_free_run_when_available=true; any other budget requires paid-funding verification. If they chose paid billing, call webhound_account and continue only when can_start_any_onboarding_run=true; otherwise explain the remaining action and wait.'
        : 'After the user says billing is ready, call webhound_account. Continue only when can_start_any_onboarding_run=true; otherwise explain the exact remaining action and wait.',
      tool: freeRunRequiresOptIn ? 'webhound_account (billing_ready branch only)' : 'webhound_account',
      wait_for_user: false,
    },
  ];
  const conversationFlow = canStart
    ? fundedFlow
    : [...fundingPrefix, ...fundedFlow.map((entry, index) => ({ ...entry, step: index + 3 }))];
  return {
...
function hostedOnboardingPlaybook(data, message, canStart, useFreeRunForFirstRun, {
  ...
  const safeDoNot = Array.isArray(interactionStyle.do_not)
    ? interactionStyle.do_not.filter(value => !/workspace|rule file|filesystem/i.test(String(value)))
    : [];
  safeDoNot.push(workspaceRulesRequested
    ? 'Do not write workspace or filesystem rules automatically. Follow the separate top-level workspace_rules approval contract, and provide an exact snippet when this hosted client cannot write the approved destination.'
    : 'Do not create, edit, or inspect workspace or filesystem rules during hosted onboarding.');
  const fundedFlow = [
  ...
  return {
    objective: source.objective
      || 'Guide the user into a useful first Webhound research run and return its inspectable result.',
    interaction_style: {
    ...
    principles: [
      ...
      workspaceRulesRequested
        ? 'The user explicitly requested separate workspace-rule guidance. Follow the top-level approval contract; never write automatically, and provide a snippet when the hosted client cannot write.'
        : 'Do not create or edit workspace rules during hosted onboarding unless the user separately and explicitly requests guidance.',
      ONBOARDING_TOPIC_CHANGE_BEHAVIOR,
    ],
    conversation_flow: conversationFlow,
...
function approvalGatedWorkspaceRules(data, capabilities, hosted) {
  const source = isRecord(data.agent_playbook?.workspace_rules)
    ? canonicalOnboardingWorkspaceRules(data.agent_playbook.workspace_rules)
    : {};
  const approvalContract = {
    ...
    reject_empty_or_frontmatter_only: true,
    instruction: hosted
      ? 'The user explicitly requested workspace-rule guidance as a separate action. Ask for the exact destination, show the complete proposed content, and obtain explicit approval before any write. If writing is unsupported, provide the exact snippet instead.'
      : 'Ask for the exact destination, show the complete proposed content, and obtain explicit approval before any write.',
...
function clientAwareOnboarding(data = {}, {
  ...
  // A caller that omits its client identity must get the filesystem-safe flow.
  // Only an explicit local-agent identity may receive workspace setup material.
  const local = LOCAL_ONBOARDING_CLIENTS.has(resolvedClient);
  const hosted = !local;
  const rawAccount = isRecord(data.account_state) ? data.account_state : {};
  const billing = isRecord(data.billing)
    ? data.billing
    : isRecord(rawAccount.billing) ? rawAccount.billing : {};
  const freeRun = isRecord(data.free_run)
    ? data.free_run
    : isRecord(rawAccount.free_run) ? rawAccount.free_run : {};
  const sourceDefaults = isRecord(data.recommended_defaults)
    ? normalizeOnboardingTierLanguage(data.recommended_defaults)
    : isRecord(rawAccount.defaults) ? normalizeOnboardingTierLanguage(rawAccount.defaults) : {};
  ...
  const creditAvailabilityVerified = (
    ...
    || rawAccount.credit_availability_verified === true
  ) && availableCreditsValue !== null;
  const availableCredits = creditAvailabilityVerified ? availableCreditsValue : 0;
  const uninterrupted = rawAccount.billing_configured_for_uninterrupted_runs === true
    || (
      billing.has_card_on_file === true
      && billing.auto_recharge_enabled === true
      && billing.auto_recharge_blocked !== true
    );
  const includedRunAutoUseEnabled = freeRun.available === true
    && recommendedDefaults.use_free_run_when_available === true
    && rawAccount.included_run_auto_use_enabled !== false;
  const savedDefaultBudget = recommendedDefaults.default_budget_usd;
  const standardPaidRunReady = rawAccount.can_start_standard_onboarding_paid_run === true
    || availableCredits >= STANDARD_ONBOARDING_BUDGET_USD
    || uninterrupted;
  const anyPaidRunReady = rawAccount.can_start_any_onboarding_paid_run === true
    || availableCredits >= MINIMUM_ONBOARDING_BUDGET_USD
    || uninterrupted;
  const savedDefaultPaidRunReady = rawAccount.can_start_default_paid_run === true
    || availableCredits >= savedDefaultBudget
    || uninterrupted;
  const canStart = rawAccount.can_start_any_onboarding_run === true
    || includedRunAutoUseEnabled
    || anyPaidRunReady;
  const canStartSavedDefault = rawAccount.can_start_default_run === true
    || savedDefaultPaidRunReady
    || (includedRunAutoUseEnabled && savedDefaultBudget === STANDARD_ONBOARDING_BUDGET_USD);
  ...
  const accountState = {
    ...
    can_start_standard_onboarding_run: rawAccount.can_start_standard_onboarding_run === true
      || includedRunAutoUseEnabled
      || standardPaidRunReady,
  ...
  const fundingMessage = includedRunAutoUseEnabled
    ? 'You have an included $5 Webhound run available.'
      : availableCredits >= STANDARD_ONBOARDING_BUDGET_USD
      ? `${freeRunRequiresOptIn ? 'An included $5 run is available, but automatic use is disabled. ' : ''}You have $${availableCredits.toFixed(2)} in available credits, enough for a paid standard $5 first run${freeRunRequiresOptIn ? ' without consuming the pass' : ''}.`
      : availableCredits >= MINIMUM_ONBOARDING_BUDGET_USD
        ? `${freeRunRequiresOptIn ? 'An included $5 run is available, but automatic use is disabled. ' : ''}You have $${availableCredits.toFixed(2)} in verified available credits. You can choose a paid first-run budget from $1 up to $${availableCredits.toFixed(2)}${freeRunRequiresOptIn ? ' without consuming the included pass' : ''}, or add billing for a larger run.`
      : standardPaidRunReady
        ? `${freeRunRequiresOptIn ? 'An included $5 run is available, but automatic use is disabled. ' : ''}Billing is configured, so a paid standard $5 first run is ready${freeRunRequiresOptIn ? ' without consuming the pass' : ''}.`
        : needsFundingChoice
          ? 'You have an included $5 run, but automatic use is disabled. I will not consume it unless you explicitly choose it for this run; otherwise use Webhound Billing.'
          : `This account has $${credits.toFixed(2)} in credits and no available included run. Add credits or connect billing before starting research.`;
  const hostedBudgetPrompt = standardPaidRunReady || includedRunAutoUseEnabled
    ? 'The recommended first-run default is $5 (about 75 minutes); tell me if you want another budget.'
    : `Choose the first-run budget explicitly. Your verified paid limit right now is $${availableCredits.toFixed(2)}; do not assume or launch the $5 standard budget unless you explicitly approve an available included pass or add funding.`;
  const hostedMessage = canStart
    ? `Welcome to Webhound onboarding. ${fundingMessage} ${hostedBudgetPrompt} What should it produce: a cited report or a sourced dataset? Tell me what you want researched and any scope that matters, plus your budget.`
    : needsFundingChoice
      ? `Welcome to Webhound onboarding. ${fundingMessage} Choose whether to use the included run this time or set up paid billing.`
      : `Welcome to Webhound onboarding. ${fundingMessage} Open Webhound Billing, then tell me when billing is ready.`;
  const localFallbackMessage = canStart
    ? `Welcome to Webhound onboarding. ${fundingMessage} Before the first run, would you like to set up this workspace first or jump right in? Setup first proposes local rules for your approval, then starts research. Jump in starts the report or dataset first.`
    : needsFundingChoice
      ? `Welcome to Webhound onboarding. ${fundingMessage} Choose whether to use the included run this time or set up paid billing.`
      : `Welcome to Webhound onboarding. ${fundingMessage} Open Webhound Billing, then tell me when billing is ready.`;
  ...
  const choices = canStart
    ? (hosted ? firstArtifactChoices : setupTimingChoices)
    : needsFundingChoice
      ? [
  ...
  const agentPlaybook = hosted
    ? hostedOnboardingPlaybook(data, message, canStart, includedRunAutoUseEnabled, {
  ...
  const userFacingGuidance = {
    ...
    ...(hosted ? {
      first_run: canStart
        ? 'Ask for a cited report or sourced dataset and the research topic, then start the selected run.'
        : needsFundingChoice
          ? 'Ask whether to use the included $5 run this time or use paid billing. Never consume the pass without the explicit use_included_run choice.'
          : `Send the user to ${BILLING_URL} before starting a spend-bearing tool.`,
    } : {}),
  };
  const result = {
    ...
    client_mode: local
      ? 'local_agent'
      : HOSTED_ONBOARDING_CLIENTS.has(resolvedClient) ? 'hosted_oauth' : 'safe_default',
    ...
    next_action: canStart
      ? (hosted
          ? `Send immediate_next_message once, mark the matching conversation_flow entry consumed, wait for the user's artifact and topic choice, then continue with the next unconsumed conversation_flow entry. setup_flow is reference-only. ${ONBOARDING_TOPIC_CHANGE_BEHAVIOR}`
          : `Send immediate_next_message once, mark the matching conversation_flow entry consumed, wait for setup_first or jump_in, then continue with the next unconsumed conversation_flow entry. setup_flow is reference-only. ${ONBOARDING_TOPIC_CHANGE_BEHAVIOR}`)
      : needsFundingChoice
        ? `Send immediate_next_message once and wait for use_included_run or billing_ready. If use_included_run is chosen, treat it as explicit consent for one exact $5 report or dataset and continue with use_free_run_when_available=true; any other budget requires paid-funding verification. If billing_ready is chosen, verify with webhound_account. Continue with the next unconsumed conversation_flow entry; do not recall onboarding merely to advance the flow. setup_flow is reference-only. ${ONBOARDING_TOPIC_CHANGE_BEHAVIOR}`
...
function accountTextSummary(data = {}) {
  ...
  const availableBalance = data.credit_availability_verified === true
    ? finiteNumber(data.credits?.available_credits)
    : null;
  ...
  if (defaultBudget !== null || defaultProduct) {
    const freeRunPreference = defaults.use_free_run_when_available === true
      ? 'yes'
      : defaults.use_free_run_when_available === false ? 'no' : 'unavailable';
...
export function createWebhoundMcpServer(options = {}) {
  ...
  registerTool(server, client, 'webhound_onboarding', {
    ...
    inputSchema: {
      client: z.enum(ONBOARDING_CLIENTS).default('generic').describe('Identify the calling agent when known. Omitted or generic callers receive the research-first, filesystem-safe flow.'),
    ...
    const data = clientAwareOnboarding(await client.onboarding(), args);
  ...
  registerTool(server, client, 'webhound_set_defaults', {
    ...
    inputSchema: {
      default_budget_usd: z.number().min(1).max(500).optional(),
      default_product: z.enum(['report', 'dataset']).optional(),
      use_free_run_when_available: z.boolean().optional(),
    },
